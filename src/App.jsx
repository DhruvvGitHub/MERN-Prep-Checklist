...
export default function App() {
  ...
  const allMust = topics.flatMap((t, ti) =>
    t.items.map((item, ii) => ({ key: `${ti}-${ii}`, priority: item.priority }))
  ).filter(i => i.priority === "must");

  const doneMust = allMust.filter(i => checked[i.key]).length;
  const totalMust = allMust.length;

  const allItems = topics.flatMap((t, ti) =>
    t.items.map((_, ii) => ({ key: `${ti}-${ii}` }))
  );
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 660, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
            ...
            <span style={{
              ...
              color: "#aaa",
              fontWeight: 600,
            }}>{doneAll}/{totalAll} done</span>
          </div>
          <p style={{ margin: "0 0 14px", color: "#555", fontSize: 13 }}>
            Tap any topic to mark it studied ✓
          </p>
          ...
          <div style={{ marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#777", marginBottom: 5 }}>
              ...
              <span style={{ color: "#ff6655", fontWeight: 700 }}>{doneMust} / {totalMust}</span>
            </div>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#555", marginBottom: 5 }}>
              ...
              <span style={{ color: "#666", fontWeight: 700 }}>{doneAll} / {totalAll}</span>
            </div>
            ...
            return (
              ...
              >
                ...
                {tDone === tTotal && tTotal > 0
                  ...
                  : tDone > 0
                  ? <span style={{ marginLeft: 5, fontSize: 10, opacity: 0.75 }}>{tDone}/{tTotal}</span>
                  : null}
              </button>
            );
          })}
        ...
        <div style={{
          ...
          <span><span style={{ color: "#ff4444", fontWeight: 700 }}>MUST</span> = asked in almost every interview</span>
          <span><span style={{ color: "#555", fontWeight: 700 }}>GOOD</span> = bonus points</span>
        </div>
      </div>
    </div>
  );
